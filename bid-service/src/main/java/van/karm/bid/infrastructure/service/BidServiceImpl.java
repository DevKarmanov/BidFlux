...
public class BidServiceImpl implements BidService {
    ...
    private final FieldEnricher bidFieldEnricher;
    private final FieldSanitizer bidFieldSanitizer;

    @Override
    public void addBid(Jwt jwt, AddBid bid) {
        String userId = jwt.getClaim("userId");

        var result = Optional.ofNullable(auctionValidator.isValid(userId, bid))
    ...
    public PagedResponse getAllBidsByAuctionId(Jwt jwt, UUID auctionId, int size, int page, Set<String> fields) {
        ...
        var paged = queryExecutor.selectQueryByFieldPaged(Bid.class,filters, LogicalOperator.NONE,fields,allowedFieldsProvider,fieldRule,pageable);
    ...
    @Override
    public BigDecimal getLastBid(UUID auctionId,Jwt jwt) {
        UUID userId = UUID.fromString(jwt.getClaim("userId"));

        if (!bidRepository.isUserAllowedForAuction(auctionId, userId)) {
            throw new AccessDeniedException("You are not allowed to access this resource");
        }

        BigDecimal lastBid = bidRepository.findLastBidAmount(auctionId);
        if (lastBid == null) {
            throw new EntityNotFoundException("No bids were found for the auction with this id");
        }
        return lastBid;
    }
}
